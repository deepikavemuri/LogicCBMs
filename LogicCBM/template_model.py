...
class End2EndModel(torch.nn.Module):
    def __init__(self, model1, model2, use_relu=False, use_sigmoid=False, n_class_attr=2, make_boolean=False):
        super(End2EndModel, self).__init__()
        self.first_model = model1
        self.sec_model = model2
        self.use_relu = use_relu
        self.use_sigmoid = use_sigmoid
        self.make_boolean = make_boolean
    ...
    def forward_stage2(self, stage1_out):
        ...
        elif self.make_boolean:
            stage1_out = torch.stack(stage1_out).squeeze()
            min_acts, max_acts = torch.min(stage1_out, dim=0), torch.max(stage1_out, dim=0)
            threshold = (min_acts[1] + max_acts[1]) / 2
            thresh_cols = []
            for i in range(stage1_out.shape[1]):
                thresh_cols.append((stage1_out[:, i] > threshold[i]).float())
            attr_outputs = torch.stack(thresh_cols, dim=1)
            attr_outputs = [attr_outputs[i].view(attr_outputs.shape[1], 1) for i in range(attr_outputs.shape[0])]
        else:
    ...
    def forward(self, x):
        if self.first_model.training:
            ...
            outs = self.forward_stage2(outputs), self.forward_stage2(aux_outputs)
            return outs
        else:
            outputs = self.first_model(x)
            return self.forward_stage2(outputs)

class End2EndLogicModel(torch.nn.Module):
    def __init__(self, model1, model2, use_relu=False, use_sigmoid=False, n_class_attr=2, use_aux_clf=False, make_boolean=False):
        super(End2EndLogicModel, self).__init__()
        self.first_model = model1.first_model
        self.first_model_classifier = model1.sec_model
        self.sec_model = model2
        self.use_relu = use_relu
        self.use_sigmoid = use_sigmoid
    ...
    def forward_stage2(self, stage1_out):
        if self.use_relu:
            ...
            stage1_out_tensor = torch.stack(stage1_out).squeeze()
            min_acts, max_acts = torch.min(stage1_out_tensor, dim=0), torch.max(stage1_out_tensor, dim=0)
            threshold = (min_acts[1] + max_acts[1]) / 2
            thresh_cols = []
            for i in range(stage1_out_tensor.shape[1]):
                thresh_cols.append((stage1_out_tensor[:, i] > threshold[i]).float())
            attr_outputs = torch.stack(thresh_cols, dim=1)
            attr_outputs = [attr_outputs[i].view(attr_outputs.shape[1], 1) for i in range(attr_outputs.shape[0])]
        else:
            attr_outputs = stage1_out
        ...
        stage2_inputs = torch.cat(stage2_inputs, dim=1)
        if self.use_aux_clf:
            outs_cbm = [self.first_model_classifier(stage2_inputs)]
        all_out = [self.sec_model(stage2_inputs)]
        if self.use_aux_clf:
            all_out.extend(outs_cbm)
        all_out.extend(stage1_out)
        return all_out
